...
export default function SeccionesPage() {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      const { data: progresoData } = await supabase
        .from('user_progress')
        .select('seccion_id, completado, aciertos, total')
        .eq('user_id', user.id)
        .eq('tema_id', temaId)
  ...
  const estaBloqueada = (index: number): boolean => {
    if (index === 0) return false
    const seccionAnterior = secciones[index - 1]
    if (!seccionAnterior) return true
    const progresoAnterior = progreso.find(p => p.seccion_id === seccionAnterior.id)
    return !progresoAnterior || !progresoAnterior.completado
  }

  const obtenerProgresoVisual = (seccionId: number) => {
    const p = progreso.find(x => x.seccion_id === seccionId)
  ...
  const obtenerEmoji = (nombre: string): string => {
    if (nombre.includes('General')) return '📋'
    if (nombre.includes('Palabra')) return '📖'
    if (nombre.includes('Eucarist')) return '🍞'
    if (nombre.includes('Conclusión')) return '🙏'
    return '📚'
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC]">
        <div className="text-center">
          ...
          <p className="text-[#64748B]">Cargando secciones...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="sticky top-0 z-20 bg-[#F8FAFC]/90 backdrop-blur-md py-4 mb-6 border-b border-[#E2E8F0] flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link 
              href="/" 
              className="w-10 h-10 flex items-center justify-center bg-white rounded-full shadow-sm hover:bg-gray-50 border border-gray-200 transition text-xl"
            >
              ⬅️
            </Link>
            <h1 className="text-xl md:text-2xl font-bold text-[#0F172A] truncate max-w-[200px] md:max-w-md">
              {temaNombre}
            </h1>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 pb-10">
          {secciones.map((seccion, index) => {
            ...
            const { pct, completado, aciertos, total } = obtenerProgresoVisual(seccion.id)

            return (
              <Link 
                key={seccion.id} 
                href={bloqueada ? '#' : `/quiz/${seccion.id}`}
                onClick={(e) => {
                  if (bloqueada) {
                    e.preventDefault()
              ...
              >
                <div className={`rounded-2xl p-5 md:p-6 border-2 transition-all h-full flex flex-col ${
                  ...
                  <div className="flex items-start gap-4 mb-4">
                    <div className={`text-2xl w-12 h-12 flex items-center justify-center rounded-xl flex-shrink-0 ${
                      bloqueada ? 'bg-gray-300' : 'bg-[#4F46E5]'
                    } text-white`}>
                      {obtenerEmoji(seccion.nombre)}
                    </div>
                    <div>
                      <h3 className={`font-bold text-lg leading-tight ${
                        bloqueada ? 'text-gray-500' : 'text-[#0F172A]'
                      }`}>
                        {seccion.nombre}
                      </h3>
                      <p className={`text-sm mt-1 ${
                        bloqueada ? 'text-gray-400' : 'text-[#64748B]'
                      }`}>
                        {bloqueada ? '🔒 Completa la anterior' : seccion.descripcion || 'Toca para comenzar'}
                      </p>
                    </div>
                  </div>
                  ...
                  {!bloqueada && (
                    <div className="mt-auto pt-2">
                      <div className="w-full bg-[#E2E8F0] rounded-full h-2.5 overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-500 ${
                            completado ? 'bg-[#10B981]' : 'bg-[#4F46E5]'
                          }`} 
                          style={{ width: `${pct}%` }} 
                        />
                      </div>
                      <div className="flex justify-between mt-2 text-xs">
                        ...
                        {pct > 0 && !completado && (
                          <span className="text-[#4F46E5]">{aciertos}/{total}</span>
                        )}
                      </div>
                    </div>
                  )}
